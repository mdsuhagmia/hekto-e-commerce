...
const Pagination = ({pageNumber, paginate, next, prev, currentPage, showFilter}) => {
  return (
    <nav aria-label="Page navigation example" className={`${showFilter.length > 0 ? "hidden" : "py-1 sm:py-4" }`}>
      <ul className="inline-flex -space-x-px text-sm sm:text-xl">
        <li className='cursor-pointer'
          onClick={prev}>
          <a  className="flex items-center justify-center px-2 sm:px-3 h-6 sm:h-8 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg">Previous</a>
        </li>
        ...
        <li className='cursor-pointer'
          onClick={next}>
          <a className="flex items-center justify-center px-2 sm:px-3 h-6 sm:h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg">Next</a>
        </li>
      </ul>
    </nav>
  )
